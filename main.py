...
class RTTSSL:
    ...
    def _handle_keyboard(self):
        ...
        if keyboard.is_pressed(keybinds.get('letter_mode', '1')) and self._can_press_key('letter_mode'):
            self.active_mode = "letter"
            self.gesture_recognizer.set_mode("letter")
            self._show_notification("Letter Translation Mode", 'info')
            
        elif keyboard.is_pressed(keybinds.get('word_mode', '2')) and self._can_press_key('word_mode'):
            self.active_mode = "word"
            self.gesture_recognizer.set_mode("word")
            self._show_notification("Word Translation Mode", 'info')
            
        elif keyboard.is_pressed(keybinds.get('gesture_mode', '3')) and self._can_press_key('gesture_mode'):
            self.active_mode = "gesture"
            self.gesture_recognizer.set_mode("gesture")
            self._show_notification("Gesture Recognition Mode", 'info')
            
        # Mouse control
        elif keyboard.is_pressed(keybinds.get('mouse_control', 'm')) and self._can_press_key('mouse_control'):
        ...
        # Face detection
        elif keyboard.is_pressed(keybinds.get('face_detection', 'f')) and self._can_press_key('face_detection'):
        ...
        elif keyboard.is_pressed(keybinds.get('toggle_detection_mode', 'e')) and self._can_press_key('toggle_detection'):
            if self.active_mode == "face":
                self.face_detector.toggle_mode()
                self._show_notification("Face Detection Mode Toggled", 'info')
                
        # Performance display
        elif keyboard.is_pressed(keybinds.get('toggle_performance', 'p')) and self._can_press_key('toggle_performance'):
            self.show_performance = not self.show_performance
            
        # FPS display
        elif keyboard.is_pressed(keybinds.get('toggle_fps', 'tab')) and self._can_press_key('toggle_fps'):
            self.show_fps = not self.show_fps
            
        # Help menu
        elif keyboard.is_pressed(keybinds.get('help_menu', 'h')) and self._can_press_key('help_menu'):
            self.show_help = not self.show_help
            
        # Recording
        elif keyboard.is_pressed(keybinds.get('toggle_recording', 'r')) and self._can_press_key('toggle_recording'):
    ...
    def _record_frame(self, frame):
        ...
        if not hasattr(self, 'video_writer'):
            ...
            if fps < 10:  # Se FPS muito baixo, use um valor fixo
                fps = 30
